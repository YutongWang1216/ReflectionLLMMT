...
def main():
    ...
    def replace_function(example):
        ...
        prompt_draft = PROMPT_DICT["prompt_draft"]

        if example.get("draft", "") != "":
            text = prompt_draft.format_map(example)
        else:
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        if not data_args.streaming:
            ...
            lm_datasets = raw_datasets.map(
                tokenize_function,
                batched=True,
                remove_columns=[text_column_name,"input","output","instruction","draft"],
            )
