...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        if self.args.lora_dim > 0:
            ...
            # print("transformer.h.0.self_attention.query_key_value.lora_left_weight: ",
            #       self.model.state_dict()["transformer.h.0.self_attention.query_key_value.lora_left_weight"])
            # print("transformer.h.0.self_attention.query_key_value.lora_right_weight: ",
            #       self.model.state_dict()["transformer.h.0.self_attention.query_key_value.lora_right_weight"])
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        # Save a trained model and configuration using `save_pretrained()`.
        # They can then be reloaded using `from_pretrained()`
        # if not isinstance(self.model.rwtranrsformer, PreTrainedModel):
        if not isinstance(self.model, PreTrainedModel):
            if isinstance(unwrap_model(self.model), PreTrainedModel):
                ...
                if self.args.lora_dim > 0:
                    ...
                    for name in state_dict:
                        if "lora" in name:
                            lora_state_dict[name] = state_dict[name]
            ...
            if self.args.lora_dim > 0:
                # save_state_dict = self.model.rwtranrsformer.state_dict()
                # print()
                # print("transformer.h.0.self_attention.query_key_value.lora_left_weight: ", self.model.state_dict()["transformer.h.0.self_attention.query_key_value.lora_left_weight"])
                # print("transformer.h.0.self_attention.query_key_value.lora_right_weight: ", self.model.state_dict()["transformer.h.0.self_attention.query_key_value.lora_right_weight"])

                # print(state_dict["transformer.h.0.self_attention.query_key_value.lora_left_weight"])
                ...
                torch.save(lora_state_dict, os.path.join(output_dir, "lora_" + WEIGHTS_NAME))
                torch.save(self.model.state_dict(), os.path.join(output_dir, "whole_"+WEIGHTS_NAME))
