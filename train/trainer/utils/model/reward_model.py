...
class RewardModel(nn.Module):
    ...
    def __init__(self, base_model, tokenizer, num_padding_at_beginning=0, rl_alpha=0.):
        super().__init__()
    ...
    def forward(self,
        ...
        lm_logits = self.rwtranrsformer.lm_head(hidden_states)
        ...
        for i in range(bs):
            ...
            # reward_diff = reward_diff.clamp(-10)
            # reward_diff = [max(i+2, 0.0) for i in reward_diff]

            loss += -torch.nn.functional.logsigmoid(reward_diff).mean()

            # list = c_truncated_reward - r_truncated_reward
            # print(self.tokenizer.decode(chosen_id[divergence_ind:end_ind]), "\nbad:", self.tokenizer.decode(rejected_id[divergence_ind:end_ind]))
            # print([i.item() for i in c_truncated_reward])
            # print([i.item() for i in torch.sigmoid(c_truncated_reward - r_truncated_reward)])
            # print(loss.item(), (end_ind - divergence_ind).item())

        # print(lm_loss.item(), (self.rl_alpha * loss / bs).item(), )
